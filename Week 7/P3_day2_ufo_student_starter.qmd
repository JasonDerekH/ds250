...
```{python}
df2 = df.assign(distance_reported = lambda x: x.distance_reported / 1000, new_size = np.where(df.estimated_size > 10000, df.estimated_size / 144, df.estimated_size))

#alternative
df['new_size'] = np.where(df.city.isin(['Holyoke', 'Crater Lake','Los Angeles', 'San Diego', 'Dallas']),df.estimated_size / 144, df.estimated_size)
```
...
```
Let's plot the mean_size by shape
...
To sort not alphabetically and not by value, but by some other variable


```{python}
mycategories = ['CIGAR', 'CIRCLE', 'CYLINDER', 'DISK', 'OVAL', 'RECTANGLE', 'SPHERE', 'TRIANGLE', 'FIREBALL', 'FORMATION', 'LIGHT', 'OTHER', 'missing']

df_spence['shape_cat'] = pd.Categorical(df_spence['shape_reported'], categories = mycategories)
```
...
.melt() makes wide data potentially longer and thinner
...
```{python}
ggplot(data = df_long) + geom_bar(aes(x = 'shape_cat', y = 'size', fill = 'source'), stat = 'identity', position = 'dodge') + 
scale_y_continuous(limits = [3000, 6000])
```